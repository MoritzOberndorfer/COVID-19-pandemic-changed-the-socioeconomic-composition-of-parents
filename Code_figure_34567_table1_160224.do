...
Code for Figures and tables in main manuscript:

Producing estimates shown in Figures 3,4,5
Figure 3
Figure 4
Figure 5

Producing estimates shown in Figures 6,7,
Figure 6
Figure 7
...
Top and bottom income quintile: Finland, Netherlands
Area deprivation: England, Scotland, Wales, Brazil, Ecuador
...
gen week=0
replace week=1 if country=="Brazil" | country=="Finland" | country=="Ecuador" | country=="Mexico" 
...
grc1leg2 AT BRA COL ECU ENG FI MEX NET Scot ESP US WAL , ///
row(4) legscale(*0.7) ///
...
plotregion(margin(zero)) imargin(zero) ///
xtob1title ytol1title ///
title("Number of Live Births in Socioeconomically Advantaged Groups", size(medsmall)) 

graph save "$graph/top_number_all$S_DATE.gph", replace
graph export "$graph/top_number_all$S_DATE.svg", replace height(2800) width(3000)
...
plotregion(margin(zero)) imargin(zero) ///
xtob1title ytol1title ///
title("Number of Live Births in Socioeconomically Disadvantaged Groups", size(medsmall)) 

graph save "$graph/bot_number_all$S_DATE.gph", replace
graph export "$graph/bot_number_all$S_DATE.svg", replace height(2800) width(3000)
